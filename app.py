...
def call_openai(prompt, max_tokens=140, temperature=0.3):
    """Direct HTTPS call; works with project-scoped keys. Kept private from UI."""
...
# Emission factors (illustrative; refine in pilot as needed)
emission_factors = {
...
bld_cost, flt_cost, wst_cost = bld["cost_$"].sum(), flt["cost_$"].sum(), wst["cost_$"].sum()
grand_t = bld_t + flt_t + wst_t
grand_cost = bld_cost + flt_cost + wst_cost
...
k5.metric("Estimated Annual Cost (CAD)", f"${grand_cost:,.0f}")
...
with tabs[1]:
    ...
    with sA:
        ...
        bld_sc_cost  = ((bld["kwh"]*elec_rate + bld["natural_gas_m3"]*gas_rate) * (1-retro/100)).sum()
        comp = pd.DataFrame({"Case":["Current","Scenario"],
                             "tCO2e":[bld_t,bld_sc_em],
                             "Cost_$":[bld_cost,bld_sc_cost]})
        st.altair_chart(
        ...
        st.caption(f"Savings: **{fmt(bld_t - bld_sc_em)} tCO₂e**, **${(bld_cost - bld_sc_cost):,.0f}**")
    ...
    with sB:
        ...
        flt_sc_cost  = flt_cost*(1-ev/100)  # proportional demo assumption
        comp = pd.DataFrame({"Case":["Current","Scenario"],
                             "tCO2e":[flt_t,flt_sc_em],
                             "Cost_$":[flt_cost,flt_sc_cost]})
        st.altair_chart(
            alt.Chart(comp.melt("Case", var_name="Metric", value_name="Value")).mark_bar().encode(
                x=alt.X("Case:N", title=""),
                y=alt.Y("Value:Q", title="Value"),
        ...
        st.caption(f"Savings: **{fmt(flt_t - flt_sc_em)} tCO₂e**, **${(flt_cost - flt_sc_cost):,.0f}**")
    ...
    with sC:
        ...
        msw_cost = wst[wst["stream"].str.contains("Municipal")]["cost_$"].sum()
        wst_sc_em = wst_t - msw_t*(div/100)
        wst_sc_cost  = wst_cost - msw_cost*(div/100)
        comp = pd.DataFrame({"Case":["Current","Scenario"],
                             "tCO2e":[wst_t,wst_sc_em],
                             "Cost_$":[wst_cost,wst_sc_cost]})
        st.altair_chart(
            alt.Chart(comp.melt("Case", var_name="Metric", value_name="Value")).mark_bar().encode(
                x=alt.X("Case:N", title=""),
                y=alt.Y("Value:Q", title="Value"),
        ...
        st.caption(f"Savings: **{fmt(wst_t - wst_sc_em)} tCO₂e**, **${(wst_cost - wst_sc_cost):,.0f}**")

# Track simple engagement badges
if "ran_scenario" not in st.session_state:
    st.session_state.ran_scenario = False
if any(k in st.session_state for k in ["retro","ev","div"]):
...
with tabs[2]:
    ...
    with left:
        ...
        st.caption("Pick jurisdiction → program. Links are clickable.")
...
3) Summary (150–250 words)
   ...
   • Current (demo): {fmt(grand_t)} tCO₂e; Est. annual cost ${grand_cost:,.0f}.
...
# ================= ENGAGEMENT (two-column tailored; share) =================
with tabs[3]:
    ...
    base_ctx = (
        f"Corporate estimate (demo) ≈ {fmt(grand_t)} tCO2e with ≈ ${grand_cost:,.0f} annual operating costs. "
    ...
    # two columns (avoid vertical scrolling)
